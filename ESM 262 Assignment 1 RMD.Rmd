...
#2/3. Create parcels tibble with only certian columns. Change na values. Coonvert columns to appropriate type
```{r}
...
parcels<- parcels_raw %>% 
  ...
  mutate(Net_AV, ifelse(Net_AV<0, NA, Net_AV))#remove any negative assessment values
...
1. 10 most frequently occurring land uses (descending)
```{r}
