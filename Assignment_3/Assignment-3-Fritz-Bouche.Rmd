...
nsample = 100
speeds = rnorm(mean=27, sd=10, n=nsample)
...
# generate sample speeds from a distribution with 10% sd
nsample_cr = 100
speeds_cr = rnorm(mean=27, sd=10, n=nsample_cr)
...
# generate sample speeds from a distribution with 10% sd
nsample_sp = 100
speeds_sp = rnorm(mean=22.4, sd=8, n=nsample_sp)
...
Based on our analysis, the mean power used over a normal distribution of car weight and surface areas, assuming a speed limit of 100km/h with standard deviation of 10%, rolling coefficient due to friction of 0.15, oefficient due to drag of 0.3, and air pressure of 1.2 kg/m^3 is `r mean(results$power)`w. If we reduce the rolling coefficient/friction of the road by half (to 0.075) the mean power is reduced to `r mean(results_cr$power)`w. Alternatively, if we reduce the speed limit of the road to 80 km/h the mean power is reduced to `r mean(results_sp$power)`w. Lowering the speed limit is less effective at reducing power needed, however this technique would be less expensive to implement. 